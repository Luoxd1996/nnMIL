...
#     @staticmethod
#     def _cover_indices(D, keep, device, shuffle=True, seed=42):
...
#         stride = max(1, keep//4)  # stride = keep//4 for large scale dataste and keep//2 for small scale dataset #
...
#         """
#         x: [B, N, D]
#         return: dict with 'logits' key (consistent with other MIL models)
#         """
#         forward_return = {}
#         B, N, D = x.shape
...
#         if self.training:
#             # Training: randomly sample 256 dimensions for attention
#             keep = min(self.H, D)
#             idx = torch.randperm(D, device=x.device)[:keep]
#             self.last_train_idx = idx  # Record for debugging
#             A = self._attend(x, idx)                          # [B,N,1]
#             feat = torch.bmm(A.transpose(1, 2), x).squeeze(1) # [B,D] full-dim pooling
...
#             if return_WSI_attn:
#                 forward_return['WSI_attn'] = A.squeeze(-1)  # [B,N]
            
#             return forward_return

#         # Validation/test: cover 256 dimensions in chunks
#         keep = min(self.H, D)
#         idx_chunks = self._cover_indices(D, keep, x.device, shuffle=self.COVER_SHUFFLE, seed=self.COVER_SEED)
...
#             if return_WSI_attn:
#                 attn_blk = A_blk.squeeze(-1)  # [B,N]
#                 attn_sum.append(attn_blk)
#         if is_cox:
#             eta_stack = torch.stack(logit_sum, dim=0)        # [K, B]
#             final_logits = torch.logsumexp(eta_stack, dim=0) - torch.log(torch.tensor(len(eta_stack), dtype=torch.float32, device=eta_stack.device))  # [B]
#         else:
#             final_logits = torch.stack(logit_sum).mean(dim=0)
...
# # model = SimpleMIL(input_dim=1536, hidden_dim=512, pred_num=2, activation='softmax', dropout=0.25)
# # model.train()
# # x = torch.randn(1, 1536, 1536)
# # with torch.no_grad():
# #     # model.eval()
# #     forward_return = model(x, return_WSI_feature=True, return_WSI_attn=True)
# #     forward_return2 = model(x, return_WSI_feature=True, return_WSI_attn=True)
# # print(forward_return['WSI_feature'].shape)
# # print(forward_return['WSI_attn'].shape)
# # print(forward_return["logits"]-forward_return2["logits"])
